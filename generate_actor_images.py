"""
Generate actor headshot images from prompts using InvokeAI.

Reads prompt files from actor_image_prompt/{actor_id}.prompt.txt,
sends each to a local InvokeAI instance running FLUX.2 Klein,
and saves the resulting image as actor_image_prompt/{actor_id}.png.

Usage:
    python generate_actor_images.py
    python generate_actor_images.py --invokeai http://localhost:9090
"""
...
def find_prompt_files(prompt_dir):
    """Find all prompt files and return sorted list of (actor_id, filepath) tuples."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate actor headshot images from prompts using InvokeAI"
    )
    parser.add_argument(
    ...
    for actor_id, prompt_path in prompt_files:
        output_path = os.path.join(PROMPT_DIR, f"{actor_id}.png")

        if os.path.exists(output_path):
            log(f"[{actor_id}] skipped (image already exists)")
            skip_count += 1
            continue
        ...
        if not prompt_text:
            log(f"[{actor_id}] skipped (empty prompt file)", "warning")
            skip_count += 1
            continue

        log(f"[{actor_id}] Generating headshot...")
        ...
        try:
            ...
            if not batch_id:
                log(f"[{actor_id}] No batch_id in enqueue response", "error")
                fail_count += 1
                continue
        except Exception as e:
            log(f"[{actor_id}] Failed to enqueue: {e}", "error")
            fail_count += 1
            continue
        ...
        if not result["success"]:
            log(f"[{actor_id}] Generation failed: {result['status']}", "error")
            fail_count += 1
            continue
        ...
        try:
            ...
            if not image_name:
                log(f"[{actor_id}] Could not find generated image", "error")
                fail_count += 1
                continue
            ...
            log(f"[{actor_id}] Saved {output_path}", "success")
            success_count += 1
        except Exception as e:
            log(f"[{actor_id}] Failed to download/save: {e}", "error")
            fail_count += 1
